...
the star of the original `Σ_j k_j · u_j(ℓ)`. This is the algebraic heart
of the advection cancellation: reality converts `u(-ℓ)` to `star(u(ℓ))`,
then telescoping `(k+ℓ)_j - k_j = ℓ_j` + div-free removes the surplus. -/
lemma advection_jsum_swap_eq_star
  ...
  -- Step 1: rewrite u_j(-ℓ) = star(u_j ℓ) via hReal.
  have hReal_jsum :
      (∑ j : Fin 2, (((k + ℓ) j : ℝ) : ℂ) * u j (-ℓ))
    ...
    intros j _
    rw [hReal j ℓ]
  -- Step 2: split (k+ℓ)_j = k_j + ℓ_j.
  have hSplit :
      (∑ j : Fin 2, (((k + ℓ) j : ℝ) : ℂ) * star (u j ℓ))
    ...
    have hCast : (((k + ℓ) j : ℝ) : ℂ) = ((k j : ℝ) : ℂ) + ((ℓ j : ℝ) : ℂ) := by
      ...
      push_cast; rfl
    rw [hCast]; ring
  -- Step 3: ℓ-part vanishes by div-free.
  have hℓPart_zero :
      (∑ j : Fin 2, ((ℓ j : ℝ) : ℂ) * star (u j ℓ)) = 0 := by
    have hDF_ℓ := hDivFree ℓ  -- Σ_j (ℓ_j : ℂ) · u j ℓ = 0
    have hStarSum : star (∑ j : Fin 2, ((ℓ j : ℝ) : ℂ) * u j ℓ) = 0 := by
      rw [hDF_ℓ]; exact star_zero _
    rw [map_sum] at hStarSum
    rw [show (∑ j : Fin 2, ((ℓ j : ℝ) : ℂ) * star (u j ℓ))
          = ∑ j : Fin 2, star (((ℓ j : ℝ) : ℂ) * u j ℓ) from ?_, hStarSum]
    apply Finset.sum_congr rfl
    intros j _
    rw [star_mul, Complex.conj_ofReal]
    ring
  -- Step 4: k-part equals star(U).
  have hKPart_eq :
      (∑ j : Fin 2, ((k j : ℝ) : ℂ) * star (u j ℓ))
        = star (∑ j : Fin 2, ((k j : ℝ) : ℂ) * u j ℓ) := by
    rw [map_sum]
    ...
    intros j _
    rw [star_mul, Complex.conj_ofReal]
  rw [hReal_jsum, hSplit, hℓPart_zero, add_zero, hKPart_eq]
...
advection cancellation theorem. Proved via the key j-sum identity
(`advection_jsum_swap_eq_star`) followed by `ring`. -/
theorem advectionSummand_swap_add_star_eq_zero
  ...
  -- For the swapped pair (k+ℓ, -ℓ), the sum is k+ℓ+(-ℓ) = k.
  rw [show (k + ℓ) + -ℓ = k from by abel]
  -- Apply the key j-sum identity.
  rw [advection_jsum_swap_eq_star hDivFree hReal]
  -- Now both terms share `star U` in the j-sum slot. Use star distribution.
  -- Key simp lemmas: Complex.conj_I, Complex.conj_ofReal (for real norms),
  -- star_star, and star distributing over multiplication.
  simp only [star_mul, Complex.conj_I, Complex.conj_ofReal, star_star,
             map_pow]
  ring
